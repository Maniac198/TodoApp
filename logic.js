...
function showTodo() {
    ...
    if (todoObj.length >= 1) {
        ...
        displayTodo.innerHTML = `<h5>There is no Todo's to show ! Add new Todo's :)</h>`;
    }
}
...
searchbtn.addEventListener("click", () => {
    ...
    Array.from(checkvalue).forEach((element) => {
        let para = element.getElementsByTagName("p")[0].innerText.toLowerCase();
        if (!para.includes(searchBoxValue)) {
            displayTodo.innerHTML = `<h5>Oops! there is nothing regarding your search</h5>`
        }
    })
